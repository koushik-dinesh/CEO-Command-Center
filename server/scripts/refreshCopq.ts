...
import { ProcessingStatus, SourceType } from '../src/db/types.js';
...
async function main() {
  ...
  if (!source) throw new Error('COPQ_DASHBOARD_SHEET not found');
  ...
  await transaction(async (connection) => {
    ...
    if (copqContext) {
      for (const record of normalized.accepted) {
        ...
        if (!ncRecords?.values?.length) continue;
  ...
  const copq = persisted.find((row) => row.metadataJson && (row.metadataJson as Record<string, unknown>).sourceCell);
  console.log('\n=== KPI VALUE PERSISTED ===');
  console.log(JSON.stringify({
    valueDecimal: copq?.valueDecimal,
    previousValueDecimal: copq?.previousValueDecimal,
    sourceCell: (copq?.metadataJson as Record<string, unknown> | undefined)?.sourceCell,
    copqYtd: (copq?.metadataJson as Record<string, unknown> | undefined)?.copqYtd,
    copqMtd: (copq?.metadataJson as Record<string, unknown> | undefined)?.copqMtd,
    copqQtd: (copq?.metadataJson as Record<string, unknown> | undefined)?.copqQtd,
  }, null, 2));
}
