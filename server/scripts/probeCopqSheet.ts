...
async function main() {
  ...
  const dbRow = await queryOne<RowDataPacket & { locationRef: string; configJson: unknown }>(
    `SELECT locationRef, configJson FROM data_sources WHERE code = 'COPQ_DASHBOARD_SHEET' LIMIT 1`,
  );
  console.log('\nDB locationRef:', dbRow?.locationRef ?? 'not found');
  console.log('DB config dashboardSheetName:', (dbRow?.configJson as { dashboardSheetName?: string } | undefined)?.dashboardSheetName ?? 'default Dashboard');
}
