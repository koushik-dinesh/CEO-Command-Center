import type { DeadSlowStockPayload, ParsedFilename } from '../types.js';
...
function isSlow(status: string): boolean {
  return /slow/i.test(status);
}

function buildAgingBuckets(problemRows: Array<{ daysIdle: number; stockValue: number }>) {
  return [
...
export function processDeadSlowStock(content: string, _meta: ParsedFilename): DeadSlowStockPayload {
