import type { ParsedFilename, RevenueSalespersonPayload } from '../types.js';
import { num, parseCsvRows, pct } from './utils.js';

export function processRevenueSalesperson(content: string, _meta: ParsedFilename): RevenueSalespersonPayload {
  ...
  for (const row of rows) row.contributionPct = pct(row.ytd, totalYtd);
