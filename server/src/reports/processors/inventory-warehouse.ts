import type { InventoryWarehousePayload, ParsedFilename } from '../types.js';
import { num, parseCsvRows, pct } from './utils.js';

export function processInventoryWarehouse(content: string, _meta: ParsedFilename): InventoryWarehousePayload {
  ...
  for (const row of rows) {
    if (!row.contributionPct) row.contributionPct = pct(row.stockValue, totalValue);
  }
