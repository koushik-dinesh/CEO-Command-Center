import { IngestionService, type IngestionRunOptions, type IngestionRunResult } from '../ingestion/IngestionService.js';
import { persistSyncSession, pruneSyncHistory } from '../ingestion/syncHistoryService.js';
import type { SyncSessionType } from '../repositories/SyncSessionRepository.js';
import { logger } from '../utils/logger.js';

export interface UnifiedSyncRunOptions extends IngestionRunOptions {
  syncType: SyncSessionType;
}

export interface UnifiedSyncRunResult extends IngestionRunResult {
  recalculatedKpiCodes: string[];
  changedSourceCodes: string[];
}

export class UnifiedSyncService {
  private ingestionService = new IngestionService();

  async run(options: UnifiedSyncRunOptions): Promise<UnifiedSyncRunResult> {
