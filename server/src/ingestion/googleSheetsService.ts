...
interface CellSnapshot {
  ...
  formula: string | null;
  valueType: 'formula' | 'static' | 'empty';
}

function serializeExtendedValue(value: unknown): unknown {
  if (!value || typeof value !== 'object') return null;
  const extendedValue = value as {
    numberValue?: number;
    stringValue?: string;
    boolValue?: boolean;
    formulaValue?: string;
  ...
  if (extendedValue.numberValue !== undefined) return extendedValue.numberValue;
  if (extendedValue.stringValue !== undefined) return extendedValue.stringValue;
  if (extendedValue.boolValue !== undefined) return extendedValue.boolValue;
  if (extendedValue.formulaValue !== undefined) return extendedValue.formulaValue;
  if (extendedValue.errorValue !== undefined) return extendedValue.errorValue;
  return null;
}

function parseCellRef(ref: string): { rowIndex: number; columnIndex: number } {
  const match = /^([A-Z]+)(\d+)$/i.exec(ref);
  if (!match) throw new Error(`Invalid Google Sheets cell reference: ${ref}`);
...
function cellAt(rowData: sheets_v4.Schema$RowData[], startRow: number, startColumn: number, ref: string): CellSnapshot {
...
export class GoogleSheetsService {
  ...
  async fetchRange(source: DataSource): Promise<SourcePayload> {
