...
import type { KpiCalculationContext, KpiCalculationResult, KpiCalculator, KpiSourceRecord } from './types.js';

function records(context: KpiCalculationContext, sourceCodes: string[]): KpiSourceRecord[] {
  return sourceCodes.flatMap((sourceCode) => context.recordsBySource.get(sourceCode) ?? []);
}

function sumMetric(context: KpiCalculationContext, sourceCodes: string[], metric: string): Decimal {
...
function current(value: Decimal): KpiCalculationResult {
  return { value, status: KpiStatus.CURRENT };
}

function unavailable(message: string): KpiCalculationResult {
...
function revenueFromSnapshot(context: KpiCalculationContext): KpiCalculationResult | null {
  const snapshot = context.snapshotContext;
  if (!snapshot?.metrics.revenue) return null;

  const revenueYtd = new Decimal(snapshot.metrics.revenue);
  if (revenueYtd.equals(0)) return null;
...
function latestRecord(context: KpiCalculationContext, sourceCode: string): KpiSourceRecord | undefined {
  return [...(context.recordsBySource.get(sourceCode) ?? [])].sort((a, b) => b.sourceDate.getTime() - a.sourceDate.getTime())[0];
}

export const calculators: KpiCalculator[] = [
  {
    code: 'REVENUE',
    requiredSources: [],
    calculate: (context) => {
  ...
  },
  {
    ...
    calculate: (context) => {
      ...
      if (snapshotRevenue == null) return unavailable('No snapshot revenue metrics available');

      const revenue = new Decimal(snapshotRevenue);
      const hrCost = sumMetric(context, ['HR_COST_SHEET'], 'hrCost');
      if (revenue.equals(0)) return unavailable('No revenue rows available');
      if (hrCost.equals(0)) return unavailable('HR cost is missing or zero');
      return {
