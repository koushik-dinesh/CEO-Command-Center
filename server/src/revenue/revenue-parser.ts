...
const requiredColumns = ['Salesperson', 'Salesperson Code', 'YTD Revenue (?)'] as const;

interface RawRevenueRow {
  [key: string]: string | undefined;
}

function assertRequiredColumns(row: RawRevenueRow | undefined): asserts row is RawRevenueRow {
  if (!row) throw new Error('Revenue Salesperson CSV is empty');
  const missing = requiredColumns.filter((column) => !(column in row));
  if (missing.length > 0) throw new Error(`Revenue Salesperson CSV is missing required columns: ${missing.join(', ')}`);
}
...
export function parseRevenueSalespersonCsv(content: string, sourceFile: RevenueSourceFile): RevenueDrilldownResponse {
  ...
  const rowsWithRevenue = rawRows.map((row, index) => {
    ...
    if (!salespersonName) throw new Error(`Revenue Salesperson CSV row ${index + 2} is missing Salesperson`);
    const revenue = toDecimal(row['YTD Revenue (?)']);
    if (!revenue) throw new Error(`Revenue Salesperson CSV row ${index + 2} has invalid YTD Revenue (?)`);
    return {
      salespersonName,
  ...
  if (rowsWithRevenue.length === 0) throw new Error('Revenue Salesperson CSV has no salesperson rows');

  const totalRevenue = rowsWithRevenue.reduce((total, row) => total.plus(row.revenue), new Decimal(0));
  if (totalRevenue.equals(0)) throw new Error('Revenue Salesperson CSV total revenue is zero');

  const rows: RevenueSalespersonRow[] = rowsWithRevenue
    .map((row) => ({
