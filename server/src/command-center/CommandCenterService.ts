...
import {
  buildInventoryDaysIntelligence,
  buildInventoryDaysKpi,
  buildInventoryDaysSummaryBullets,
  buildInventoryDaysTrends,
  buildInventoryKpi,
  resolveInventorySnapshotDate,
} from './inventoryDays.js';
...
function payloadMap(batch: ReportSnapshotRow[]) {
  const map = new Map<string, ReportSnapshotRow>();
  for (const row of batch) map.set(row.reportType, row);
  ...
  return {
    salesperson: map.get('REVENUE_BY_SALESPERSON')?.payloadJson as RevenueSalespersonPayload | undefined,
    customerGroup: map.get('REVENUE_BY_CUSTOMER_GROUP')?.payloadJson as RevenueCustomerGroupPayload | undefined,
    productGroup: map.get('REVENUE_BY_PRODUCT_GROUP')?.payloadJson as RevenueProductGroupPayload | undefined,
    revenueVsCogs: map.get('REVENUE_VS_COGS')?.payloadJson as RevenueVsCogsPayload | undefined,
    inventory: map.get('INVENTORY_BY_WAREHOUSE')?.payloadJson as InventoryWarehousePayload | undefined,
    deadStock: map.get('DEAD_SLOW_MOVING_STOCK')?.payloadJson as DeadSlowStockPayload | undefined,
...
function filterSalesperson(payload: RevenueSalespersonPayload | undefined, filters: CommandCenterFilters) {
  if (!payload || !filters.salesperson) return payload ?? null;
...
function buildCustomerGroupInsights(
  customerGroup: RevenueCustomerGroupPayload | null,
  previousCustomerGroup: RevenueCustomerGroupPayload | null,
) {
...
function buildRevenueIntelligence(
  ...
  revenueHistory: MetricTrendPoint[],
...
function buildProfitability(revenueVsCogs: RevenueVsCogsPayload | null, marginHistory: MetricTrendPoint[]): ProfitabilityIntelligence {
...
function buildInventory(inventory: InventoryWarehousePayload | null, inventoryHistory: MetricTrendPoint[], filters: CommandCenterFilters): InventoryIntelligence {
...
function buildDeadStock(deadStock: DeadSlowStockPayload | null, deadHistory: Array<{ snapshotKey: string; snapshotDate: string; dead: number; slow: number }>): DeadStockIntelligence {
  return {
...
async function buildHistoriesFromMetrics() {
  ...
  const revenueHistory: MetricTrendPoint[] = rows
  ...
  const grossProfitHistory: MetricTrendPoint[] = rows.map((row) => ({
    snapshotKey: row.snapshotKey,
    snapshotDate: row.snapshotDate,
    value: row.grossProfit ?? 0,
  }));

  const marginHistory: MetricTrendPoint[] = rows.map((row) => ({
    snapshotKey: row.snapshotKey,
    snapshotDate: row.snapshotDate,
    value: row.grossMargin ?? 0,
  }));

  const inventoryHistory: MetricTrendPoint[] = rows.map((row) => ({
    snapshotKey: row.snapshotKey,
    snapshotDate: row.snapshotDate,
...
function metricsRowToCore(row: SnapshotMetricsRow | null) {
  if (!row) return null;
  return {
    revenue: row.revenue,
...
function resolveCoreMetrics(
  metricsRow: SnapshotMetricsRow | null,
  batch: ReturnType<typeof payloadMap>,
) {
...
async function buildHistories() {
  ...
  if (metricsCount > 0) return buildHistoriesFromMetrics();
...
async function buildHistoriesFallback() {
  ...
  const revenueHistory: MetricTrendPoint[] = revenueRows
    .map((row) => {
      const payload = row.payloadJson as RevenueVsCogsPayload;
      const value = payload.total?.ytdRevenue ?? 0;
      return { snapshotKey: row.snapshotKey, snapshotDate: row.snapshotDate, value };
    })
    .filter((point) => point.value > 0);

  const grossProfitHistory: MetricTrendPoint[] = revenueRows.map((row) => {
    const payload = row.payloadJson as RevenueVsCogsPayload;
  ...
  const marginHistory: MetricTrendPoint[] = cogsRows.map((row) => {
    const payload = row.payloadJson as RevenueVsCogsPayload;
    return { snapshotKey: row.snapshotKey, snapshotDate: row.snapshotDate, value: payload.total?.grossProfitPct ?? 0 };
  });

  const inventoryHistory: MetricTrendPoint[] = inventoryRows.map((row) => {
    const payload = row.payloadJson as InventoryWarehousePayload;
  ...
  const deadHistory = deadRows.map((row) => {
    const payload = row.payloadJson as DeadSlowStockPayload;
...
export class CommandCenterService {
  async resolveSnapshotKey(snapshotKey?: string): Promise<string | null> {
  ...
  async getDashboard(snapshotKey?: string, filters: CommandCenterFilters = {}): Promise<CommandCenterResponse> {
    const resolvedKey = await this.resolveSnapshotKey(snapshotKey);
    if (!resolvedKey) throw new Error('No snapshots available. Run data refresh to sync from Google Drive.');
    ...
    const kpis: CommandCenterResponse['kpis'] = [
      buildKpi(
        'revenue',
        'Revenue',
        ...
        'currency',
        histories.revenueHistory,
        undefined,
        buildRevenueSubMetrics(revenuePeriods),
      ),
      ...
      buildKpi(
        'grossMargin',
        'Gross Margin',
        currentMetrics.grossMarginPct,
        previousMetrics.grossMarginPct,
        'percent',
        histories.marginHistory,
        currentMetrics.grossMarginPct && currentMetrics.grossMarginPct >= 40 ? 'good' : 'warning',
        undefined,
    ...
    if (pbtKpi) kpis.splice(3, 0, pbtKpi);
    ...
    const revenueIntel = buildRevenueIntelligence(
      current.salesperson ?? null,
      current.customerGroup ?? null,
      previous.customerGroup ?? null,
    ...
    }, (apiCopqKpi?.metadata ?? {}) as Record<string, unknown>);

    return response;
  }

  async compare(currentSnapshotKey: string, previousSnapshotKey: string): Promise<ComparisonResponse> {
