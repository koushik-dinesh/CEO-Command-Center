...
import { parseNcCopqRecords, type NcCopqAnalyticsRecord, type NcRecordsParseConfig } from './ncRecords.js';
...
export interface PersistCopqFactsInput {
  dataSourceId: string;
  ncValues: unknown[][];
  ncSheetName?: string | null;
  normalized: unknown;
...
export class CopqAnalyticsService {
  static async persistFromNcValues(
    input: PersistCopqFactsInput,
    executor?: DbExecutor,
  ): Promise<{ recordCount: number; rejectedRowCount: number } | null> {
    if (!input.ncValues.length) return null;
  ...
  static async loadHeadlineIfAvailable(dataSourceId: string): Promise<Record<string, unknown> | null> {
    const meta = await CopqAnalyticsMetaRepository.findByDataSourceId(dataSourceId);
    return meta?.headlineJson ?? null;
  }

  static async loadFactsIfAvailable(dataSourceId: string): Promise<{
    records: NcCopqAnalyticsRecord[];
    meta: Awaited<ReturnType<typeof CopqAnalyticsMetaRepository.findByDataSourceId>>;
  } | null> {
    const meta = await CopqAnalyticsMetaRepository.findByDataSourceId(dataSourceId);
    if (!meta || meta.recordCount === 0) return null;
