...
const envSchema = z
  .object({
    ...
    /** Reserved for Phase 2+ unified sync scheduler; not started in Phase 1. */
    SYNC_CRON: z.string().min(1).default('0 */2 * * *'),
    ...
    SYNC_HISTORY_RETENTION_COUNT: z.coerce.number().int().positive().default(1000),
    PROCESSING_LOG_RETENTION_DAYS: z.coerce.number().int().positive().default(90),
    SNAPSHOT_SYNC_RUN_RETENTION_DAYS: z.coerce.number().int().positive().default(90),
    KPI_VALUE_HISTORY_RETENTION_DAYS: z.coerce.number().int().positive().default(180),
    KPI_VALUE_HISTORY_MAX_ROWS_PER_KPI: z.coerce.number().int().positive().default(500),
    UPLOADED_FILE_RETENTION_DAYS: z.coerce.number().int().positive().default(180),
    REPORT_SNAPSHOT_RETENTION_DAYS: z.coerce.number().int().positive().default(400),
