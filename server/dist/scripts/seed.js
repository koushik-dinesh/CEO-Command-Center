...
async function findIdByCode(table, code) {
    const row = await queryOne(`SELECT id FROM ${table} WHERE code = ? LIMIT 1`, [code]);
    if (!row)
        throw new Error(`${table} row not found for code ${code}`);
    return row.id;
}
async function main() {
    ...
    await transaction(async (connection) => {
        await execute(`INSERT INTO users (id, email, passwordHash, name, isActive)
       VALUES (?, ?, ?, 'CEO', TRUE)
       ON DUPLICATE KEY UPDATE passwordHash = VALUES(passwordHash), name = VALUES(name), isActive = TRUE, updatedAt = UTC_TIMESTAMP(3)`, [createId('user'), email, passwordHash], connection);
        for (const source of dataSources) {
            await execute(`INSERT INTO data_sources (id, code, name, sourceType, provider, locationRef, configJson, isActive)
         VALUES (?, ?, ?, ?, ?, ?, ?, TRUE)
         ON DUPLICATE KEY UPDATE name = VALUES(name), sourceType = VALUES(sourceType), provider = VALUES(provider), locationRef = VALUES(locationRef), configJson = VALUES(configJson), isActive = TRUE, updatedAt = UTC_TIMESTAMP(3)`, [createId('ds'), source.code, source.name, source.sourceType, source.provider, source.locationRef, stringifyJson(source.configJson)], connection);
        }
        await execute(`UPDATE data_sources
       SET isActive = FALSE, locationRef = ?, updatedAt = UTC_TIMESTAMP(3)
       WHERE code = 'NC_REGISTER_CSV'`, ['1Iub_F2LioHu3c0lZczrhrC-KaJlQKtmw'], connection);
        for (const kpi of kpis) {
            await execute(`INSERT INTO kpi_definitions (id, code, name, description, unit, displayFormat, calculationType, sortOrder, isActive, configJson)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, TRUE, ?)
         ON DUPLICATE KEY UPDATE name = VALUES(name), description = VALUES(description), unit = VALUES(unit), displayFormat = VALUES(displayFormat), calculationType = VALUES(calculationType), sortOrder = VALUES(sortOrder), isActive = TRUE, configJson = VALUES(configJson), updatedAt = UTC_TIMESTAMP(3)`, [createId('kpi'), kpi.code, kpi.name, kpi.description, kpi.unit, kpi.displayFormat, kpi.calculationType, kpi.sortOrder, stringifyJson({ dependencies: kpi.sources })], connection);
        }
    });
    ...
    if (existingDashboard) {
        await execute('UPDATE dashboard_configurations SET name = ?, layoutJson = ?, refreshIntervalSeconds = ?, updatedAt = UTC_TIMESTAMP(3) WHERE id = ?', ['Dashboard', stringifyJson(layoutJson), 900, existingDashboard.id]);
    }
    else {
        await execute('INSERT INTO dashboard_configurations (id, name, isDefault, layoutJson, refreshIntervalSeconds) VALUES (?, ?, TRUE, ?, ?)', [createId('dash'), 'Dashboard', stringifyJson(layoutJson), 900]);
    }
}
main()
