...
function buildParseConfig(config) {
    return {
...
function analyzeField(records, field) {
    let maxLen = 0;
    let longest = '';
...
function recommendVarchar(maxLen, headroomPct = 25) {
    const withHeadroom = Math.ceil(maxLen * (1 + headroomPct / 100));
    const tiers = [64, 128, 191, 255, 512, 1024];
    for (const tier of tiers) {
        if (withHeadroom <= tier)
            return tier;
    }
    return -1;
}
function recommendFieldLimit(maxLen) {
...
async function loadNcValuesFromSheets() {
    ...
    if (!values.length)
        throw new Error('Google Sheets returned no NC rows');
    console.log('Source: live Google Sheets NC register');
    return values;
}
async function loadNcValues() {
    try {
