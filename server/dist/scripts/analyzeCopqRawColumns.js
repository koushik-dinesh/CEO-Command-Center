...
async function main() {
    ...
    const indices = Object.fromEntries(Object.entries(COLUMN_CANDIDATES).map(([key, candidates]) => [key, findIdx(headers, candidates)]));
    ...
    const rootCauseCombined = dataRows.map((row) => {
        const primary = indices.rootCausePrimary >= 0 ? norm(row[indices.rootCausePrimary]) : '';
        const fallback = indices.rootCauseFallback >= 0 ? norm(row[indices.rootCauseFallback]) : '';
