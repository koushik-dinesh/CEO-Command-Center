...
function mapRow(row) {
    if (!row)
        return null;
    return { ...row, payloadJson: parseJsonField(row.payloadJson) };
}
export class RevenueDrilldownCacheRepository {
    static async latest() {
        const row = await queryOne('SELECT * FROM revenue_drilldown_cache ORDER BY fileTimestamp DESC, updatedAt DESC LIMIT 1');
        return mapRow(row);
    }
    static async upsert(data) {
        const id = createId('rev_cache');
        await execute(`INSERT INTO revenue_drilldown_cache
       ...
       ON DUPLICATE KEY UPDATE
        ...
        const row = await queryOne('SELECT * FROM revenue_drilldown_cache WHERE providerFileId = ? LIMIT 1', [data.providerFileId]);
        const mapped = mapRow(row);
        if (!mapped)
            throw new Error(`Revenue drilldown cache ${data.providerFileId} not found after upsert`);
        return mapped;
    }
}
