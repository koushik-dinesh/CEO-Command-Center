...
export function normalizeSnapshotDate(value) {
    ...
    if (isoPrefix?.[1] && isValidYmd(isoPrefix[1]))
        return isoPrefix[1];
    if (/^\d{8}$/.test(trimmed)) {
...
export function parseDateFromSnapshotKey(snapshotKey) {
    if (!snapshotKey)
        return null;
...
function candidateSources(batch, preferredReportType) {
    ...
    for (const row of orderedRows) {
        candidates.push({ label: `${row.reportType}.snapshotDate`, value: row.snapshotDate }, { label: `${row.reportType}.fileName`, value: row.fileName }, { label: `${row.reportType}.snapshotKey`, value: row.snapshotKey });
    }
    return candidates;
}
export function resolveSnapshotDateFromBatch(batch, options) {
...
export function formatSnapshotDateFromDb(value) {
