...
export function isDiscoveryWindowMinute(now = new Date(), timeZone = getTimezone()) {
...
export function isDiscoveryWindowExhausted(now = new Date(), timeZone = getTimezone()) {
    const minutes = minutesSinceMidnight(now, timeZone);
    const end = parseTimeToMinutes(env.SNAPSHOT_DISCOVERY_END);
    return minutes > end;
}
export function todayDateKey(now = new Date(), timeZone = getTimezone()) {
