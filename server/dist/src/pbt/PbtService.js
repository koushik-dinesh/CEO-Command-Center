...
import { PbtMonthlyInputRepository } from '../repositories/PbtMonthlyInputRepository.js';
...
export function computeTotalIndirectExpense(hrExpense, additionalIndirectExpense) {
    return (hrExpense ?? 0) + (additionalIndirectExpense ?? 0);
}
export function calculatePbt(revenue, directExpense, totalIndirectExpense) {
    if (revenue === null || directExpense === null || totalIndirectExpense === null)
        return null;
    return revenue - (directExpense + totalIndirectExpense);
}
async function getHrExpenseForMonth(month, year) {
    const row = await HrExpenseRepository.findByCalendarMonth(month, year);
    return row?.hrExpense ?? null;
}
export function validateExpenseValue(value, fieldName) {
...
async function getRevenueForMonth(month, year) {
...
async function toCalculatedRecord(input, month, year, revenue) {
...
function generatePbtInsights(records) {
    const insights = [];
    if (records.length < 2)
        return insights;
...
function pbtHealth(value, changePercent) {
    if (value === null)
        return 'neutral';
    if (value < 0)
        return 'critical';
    if (changePercent !== null && changePercent < -5)
        return 'warning';
    if (value > 0 && (changePercent === null || changePercent >= 0))
        return 'good';
    return 'neutral';
}
export class PbtService {
    async getRevenueForMonth(month, year) {
    ...
    async getHrExpenseForMonth(month, year) {
    ...
    async createInput(userId, month, year, directExpense, additionalIndirectExpense) {
    ...
    async updateInput(month, year, directExpense, additionalIndirectExpense) {
        const existing = await PbtMonthlyInputRepository.findByMonthYear(month, year);
        if (!existing) {
    ...
    async buildKpi() {
        ...
        if (!latestInput) {
            return buildKpi('pbt', 'Profit Before Tax', null, null, 'currency', [], 'neutral', [
        ...
        const kpi = buildKpi('pbt', 'Profit Before Tax', latest?.profitBeforeTax ?? null, previous?.profitBeforeTax ?? null, 'currency', history, pbtHealth(latest?.profitBeforeTax ?? null, changePercent), [
