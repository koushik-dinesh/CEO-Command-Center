...
function processingMetadata(source, normalized) {
    ...
    async fetchSourcePayloads(source) {
        if (source.provider === SourceProvider.GOOGLE_DRIVE && source.sourceType === SourceType.CSV) {
            if (source.code === 'REVENUE_CSV')
                return this.driveService.fetchMatchingCsvs(source);
                    ...
                    if (source.code === 'COPQ_DASHBOARD_SHEET') {
                        for (const record of normalized.accepted) {
                            logO34Stage('STAGING O34', {
                                ...
                                rawCellKeys: record.raw && typeof record.raw === 'object' && record.raw.cells
                                    ? Object.keys(record.raw.cells)
                                    : [],
                                rawTotalCopq: record.raw && typeof record.raw === 'object'
                                    ? record.raw.cells?.totalCopq ?? null
                                    : null,
                            }, record.normalized);
                        }
