...
export async function pruneAuditTables() {
    const processingLogsDeleted = await ProcessingLogRepository.pruneOlderThan(env.PROCESSING_LOG_RETENTION_DAYS);
    const [syncSessionsDeleted, snapshotSyncRunsDeleted, uploadedFilesDeleted, kpiHistoryDeletedByAge, kpiHistoryDeletedByCount, snapshotPrune,] = await Promise.all([
