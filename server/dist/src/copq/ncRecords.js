...
function cellText(value) {
    ...
    if (!text || text.toLowerCase() === 'na' || text.toLowerCase() === 'n/a')
        return '';
    return text;
}
function normalizeCategory(issueRelatedTo, department, qcType) {
...
function formatDisplayDate(value, dateKey) {
    const parsed = parseSourceDate(value);
    if (parsed) {
...
export function parseNcCopqRecords(values, config) {
    if (!values.length) {
        return {
            records: [],
            headerRowIndex: -1,
