...
import { applyKpiSemantics, getImprovementDirection, getKpiTrendConfig, healthFromTrendSentiment, resolveTrendLabel, resolveTrendSentiment, } from './kpiSemantics.js';
...
export function changeMetrics(current, previous) {
    if (current === null || previous === null) {
        return { changePercent: null, changeAbsolute: null, trend: 'UNKNOWN' };
    }
...
export function buildKpi(key, label, value, previousValue, unit, history, health, subMetrics, options) {
...
export function buildComparisonMetric(key, label, current, previous) {
...
export function generateDrilldownSummaries(input) {
    ...
    if (revChange)
        revenueBullets.push(`Revenue ${revChange} compared to the previous snapshot.`);
    if (topCustomer)
        revenueBullets.push(`${topCustomer.name} is the largest customer group at ${topCustomer.contributionPct.toFixed(1)}% of revenue.`);
    if (topSales)
        revenueBullets.push(`${topSales.name} is the top salesperson contributing ${topSales.contributionPct.toFixed(1)}% of revenue.`);
    if (topProduct)
        revenueBullets.push(`${topProduct.name} leads product groups with ${topProduct.contributionPct.toFixed(1)}% share.`);
    ...
    if (bestCategory)
        profitabilityBullets.push(`${bestCategory.type} delivers the highest margin at ${bestCategory.marginPct.toFixed(1)}%.`);
    ...
    if (gpChange)
        profitabilityBullets.push(`Gross profit ${gpChange} versus the prior period.`);
    ...
    if (invChange)
        inventoryBullets.push(`Total inventory ${invChange} compared to the previous snapshot.`);
    if (topWarehouse)
        inventoryBullets.push(`${topWarehouse.name} holds ${topWarehouse.contributionPct.toFixed(1)}% of total inventory value.`);
    const top3Share = input.inventory.byWarehouse.slice(0, 3).reduce((sum, row) => sum + row.contributionPct, 0);
    if (top3Share > 0)
        inventoryBullets.push(`Top 3 warehouses account for ${top3Share.toFixed(1)}% of inventory concentration.`);
    ...
    const deadBullets = [];
    if (deadChange)
        deadBullets.push(`Dead stock ${deadChange} versus the previous snapshot.`);
    if (slowChange)
        deadBullets.push(`Slow-moving stock ${slowChange} versus the previous snapshot.`);
...
function pickRevenueFromCogsTotal(period, total) {
    if (!total)
        return null;
    if (period === 'ytd')
        return positive(total.ytdRevenue);
    if (period === 'qtd')
        return positive(total.qtdRevenue);
    return positive(total.mtdRevenue);
}
function pickRevenuePeriod(period, payloads) {
...
function pickRevenue(payloads) {
    return pickRevenuePeriod('ytd', payloads);
}
export function extractRevenuePeriods(payloads) {
    return {
...
export function buildRevenueSubMetrics(periods) {
    return [
...
function metadataDecimal(metadata, key) {
...
export function buildCopqSubMetrics(metadata, o34Ytd = null) {
    const ytd = o34Ytd;
