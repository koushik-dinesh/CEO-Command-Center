...
import { calculateProductivityIndex, financialYearFromCalendar, financialYearLabel, monthShortLabel, monthsForMtd, monthsForQtd, monthsForYtd, referenceDateFromSnapshot, sumHrExpenses, } from './productivityUtils.js';
function toHrExpenseRecord(row) {
    return {
        id: row.id,
...
function generateInsights(current, previous, revenueYtd, hrExpenseYtd) {
    const insights = [];
    if (current === null) {
        insights.push({
...
export class ProductivityService {
    async listHrExpenses() {
        const rows = await HrExpenseRepository.listAll();
        return rows.map(toHrExpenseRecord);
    }
    async createHrExpense(userId, month, calendarYear, hrExpense) {
    ...
    async updateHrExpense(userId, month, calendarYear, hrExpense) {
        const existing = await HrExpenseRepository.findByCalendarMonth(month, calendarYear);
        if (!existing) {
    ...
    async deleteHrExpense(month, calendarYear) {
        const deleted = await HrExpenseRepository.deleteByCalendarMonth(month, calendarYear);
        if (!deleted) {
            throw Object.assign(new Error('No HR expense found for this month.'), { statusCode: 404 });
        }
    }
    async upsertHrExpense(userId, month, calendarYear, hrExpense) {
        const existing = await HrExpenseRepository.findByCalendarMonth(month, calendarYear);
        if (existing) {
    ...
    async computePeriodMetrics(referenceDate, revenuePayload, hrRecords) {
    ...
    async buildMonthlyTrend(hrRecords) {
    ...
    async buildIntelligence(input) {
        ...
        return {
            ...
            insights: generateInsights(current.productivityYtd, previous?.productivityYtd ?? null, current.revenueYtd, current.hrExpenseYtd),
        };
    }
    async buildKpi(input) {
        ...
        const footnote = current.hrExpenseYtd === null
            ...
            : null;
        const kpi = buildKpi('productivityIndex', 'Productivity Index', current.productivityYtd, previous?.productivityYtd ?? null, 'ratio', history, undefined, buildSubMetrics({
            productivityYtd: current.productivityYtd,
            productivityQtd: current.productivityQtd,
            productivityMtd: current.productivityMtd,
        }));
...
export function buildProductivitySummaryBullets(intelligence) {
    const bullets = [];
